    ...
    def gen_mgmt(self):
        ...
        if 'vEOS-lab' in self.image:
            ...
            res.append(self.nic_type + ",netdev=p%(i)02d,mac=%(mac)s"
                       % { 'i': 0, 'mac': gen_mac(0) })
        res.append("-netdev")
        res.append("user,id=p%(i)02d,net=10.0.0.0/24,tftp=/tftpboot,hostfwd=tcp::2022-10.0.0.15:22,hostfwd=udp::2161-10.0.0.15:161,hostfwd=tcp::2830-10.0.0.15:830,hostfwd=tcp::2080-10.0.0.15:80,hostfwd=tcp::28728-10.0.0.15:8728,hostfwd=tcp::2443-10.0.0.15:443" % { 'i': 0 })
...
class VR:
    ...
    def start(self):
        ...
        run_command(["socat", "TCP-LISTEN:8728,fork", "TCP:127.0.0.1:28728"], background=True)
